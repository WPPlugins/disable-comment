...
if (!class_exists('WSXDC_Settings')) {

    /**
     * Handles plugin settings and user profile meta fields
     */
    class WSXDC_Settings extends WSXDC_Module
    {
        protected $settings;
         ...
         * @param array $value This will be merged with WSXDC_Settings->settings, so it should mimic the structure of the WSXDC_Settings::$default_settings. It only needs the contain the values that will change, though. See WordPress_Disable_Comments->upgrade() for an example.
         */
        public function __set($variable, $value)
        {
            // Note: WSXDC_Module::__set() is automatically called before this
            ...
            update_option('adc_settings', $this->settings);
        }
        ...
        public function register_hook_callbacks()
        {
            if ($this->network_activated) {
                ...
                add_action('network_admin_edit_adc_settings', array($this, 'update_network_setting'));
            } else {
        ...
        {
            update_site_option('adc_settings', $_POST['adc_settings']);
            wp_redirect(add_query_arg(array('page' => adc_settings, 'updated' => 'true'), network_admin_url('settings.php')));
            exit;
        }
        ...
        protected function get_settings()
        {
            $settings = shortcode_atts(
                ...
                $this->network_activated ? get_site_option('adc_settings', array()) : get_option('adc_settings', array())
            );
        ...
        public function add_plugin_action_links($links)
        {
            ...
            if ($this->network_activated) {
                array_unshift($links, '<a href="settings.php?page=adc_settings">Settings</a>');
            } else {
                array_unshift($links, '<a href="options-general.php?page=adc_settings">Settings</a>');
            }
            return $links;
        }
        ...
        public function markup_settings_page()
        {
            if (($this->network_activated && current_user_can(self::REQUIRED_CAPABILITY_MU)) ||
                ...
                echo self::render_template('wsxdc-settings/page-settings.php', array('network_activated' => $this->network_activated), 'always');
            } else {
        ...
        {
            $this->add_settings_field($id, $title, 'adc_section-disablewhat');
        }

        private function add_settings_field_disablewhere($id, $title)
        {
            $this->add_settings_field($id, $title, 'adc_section-disablewhere');
        }
        ...
        public function register_settings()
        {
             ...
             * What Section
             */
            $this->add_settings_section('adc_section-disablewhat', 'What to disable');
            $this->add_settings_field_disablewhat('adc_disable-comments', 'Comments');
            $this->add_settings_field_disablewhat('adc_disable-pingbacks', 'Pingbacks');
            $this->add_settings_field_disablewhat('adc_disable-trackbacks', 'Trackbacks');
            $this->add_settings_field_disablewhat('adc_disable-xmlrpc', 'XML-RPC');
            $this->add_settings_field_disablewhat('adc_disable-rsdlink', 'RSD links');
            $this->add_settings_field_disablewhat('adc_disable-rcwidget', 'Recent Comments Widget in Dashboard');
            $this->add_settings_field_disablewhat('adc_disable-urlfield', 'URL Field in comment form');
            $this->add_settings_field_disablewhat('adc_disable-authorlink', 'Comment author link');
            $this->add_settings_field_disablewhat('adc_prevent-ownership', 'Comment URL with Google authorship link');

            /*
             * Where Section
             */
            $this->add_settings_section('adc_section-disablewhere', 'Where/When to disable');

            $this->add_settings_field_disablewhere('adc_disable-on-logged-in', 'for logged in users');
            $this->add_settings_field_disablewhere('adc_disable-post-id', 'for specific post/page IDs');
            $this->add_settings_field_disablewhere('adc_disable-category', 'for specific categories');
            $this->add_settings_field_disablewhere('adc_disable-tag', 'for specific tags');
            $this->add_settings_field_disablewhere('adc_disable-user', 'for specific authors');
            if (current_theme_supports('post-formats')) {
                $this->add_settings_field_disablewhere('adc_disable-format', 'for specific post formats');
            }
            $this->add_settings_field_disablewhere('adc_disable-post-type', 'for specific post types');
            $this->add_settings_field_disablewhere('adc_disable-language', 'for specific languages');
            $this->add_settings_field_disablewhere('adc_disable-url', 'for specific URL paths');
            $this->add_settings_field_disablewhere('adc_disable-referrer', 'for specific referrers');
            $this->add_settings_field_disablewhere('adc_disable-ipaddress', 'for specific IP addresses');
            $this->add_settings_field_disablewhere('adc_disable-checkboxes', 'Uncheck discussion comment checkboxes for');

            // The settings container
            register_setting('adc_settings', 'adc_settings', array($this, 'validate_settings'));
        }
        ...
        {
            echo self::render_template('wsxdc-settings/page-settings-section-headers.php', array('section' => $section), 'always');
        }
        ...
        {
            global $q_config;
            echo self::render_template('wsxdc-settings/page-settings-fields.php', array('settings' => $this->settings, 'field' => $field, 'q_config' => $q_config), 'always');
        }
